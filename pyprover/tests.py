...
# other tests
        ...
        assert (simplify)((substitute)(a ^ b, {a: True, b: False})) == top  #155 (line in Coconut source)
        assert (simplify)((substitute)(a ^ b, {a: top, b: top})) == bot  #156 (line in Coconut source)
...
def test_regression():  #373 (line in Coconut source)
    ...
    assert expr("Q(?x) & P(?y)").find_unification(expr("Q(a) & P(b)")) == {Var("x"): Const("a"), Var("y"): Const("b")}  #384 (line in Coconut source)
