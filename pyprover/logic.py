...
class Quantifier(Expr):  #476 (line num in coconut source)
    ...
    def make_free_in(self, other):  #517 (line num in coconut source)
        ...
        while not var.is_free_in(other):  #520 (line num in coconut source)
            var = var.prime()  #521 (line num in coconut source)
        return _coconut_tail_call(self.change_var, var)  #522 (line num in coconut source)


    @_coconut_tco  #524 (line num in coconut source)
    def find_unification(self, other, **kwargs):  #524 (line num in coconut source)
        unif = self.elem.find_unification(other, **kwargs)  #525 (line num in coconut source)
        if unif is None:  #526 (line num in coconut source)
            return None  #527 (line num in coconut source)
        else:  #528 (line num in coconut source)
            return _coconut_tail_call((rem_var), self.var, unif)  #529 (line num in coconut source)


    @_coconut_tco  #531 (line num in coconut source)
    def resolve_against(self, other, **kwargs):  #531 (line num in coconut source)
        if isinstance(other, Quantifier):  #532 (line num in coconut source)
            resolution = (self.elem.resolve_against)(Not(other.elem).simplify(**kwargs), **kwargs)  #533 (line num in coconut source)
            if resolution is None:  #534 (line num in coconut source)
                return None  #535 (line num in coconut source)
            elif isinstance(other, ForAll):  # don't pull an Exists out of a ForAll  #536 (line num in coconut source)
                return _coconut_tail_call((other.change_elem), (self.change_elem)(resolution))  #537 (line num in coconut source)
            else:  #538 (line num in coconut source)
                return _coconut_tail_call((self.change_elem), (other.change_elem)(resolution))  #539 (line num in coconut source)
        else:  #540 (line num in coconut source)
            __class__ = Quantifier  #541 (line num in coconut source)

            return super().resolve_against(other, **kwargs)  #541 (line num in coconut source)

    @classmethod  #542 (line num in coconut source)

    @_coconut_tco  #544 (line num in coconut source)
    def blank(cls, elem):  #544 (line num in coconut source)
        """Make a quantifier without a variable."""  #545 (line num in coconut source)
        return _coconut_tail_call(cls(empty_var, elem).make_free_in, elem)  #546 (line num in coconut source)



_coconut_call_set_names(Quantifier)  #549 (line num in coconut source)
class ForAll(Quantifier):  #549 (line num in coconut source)
    """Universal quantifier."""  #550 (line num in coconut source)
    __slots__ = ()  #551 (line num in coconut source)
    opstr = forall_sym  #552 (line num in coconut source)

    def __init__(self, var, elem):  #554 (line num in coconut source)
        assert wff(elem), elem  #555 (line num in coconut source)
        if isinstance(var, str):  #556 (line num in coconut source)
            var = Const(var)  #557 (line num in coconut source)
        assert isvar(var), var  #558 (line num in coconut source)
        self.var = var.variable()  #559 (line num in coconut source)
        self.elem = elem.substitute({var: self.var.variable()})  #560 (line num in coconut source)


    def inner_kwargs(self, kwargs):  #562 (line num in coconut source)
        __class__ = ForAll  #563 (line num in coconut source)

        inner_kwargs = super().inner_kwargs(kwargs)  #563 (line num in coconut source)
        inner_kwargs["variables"] = inner_kwargs.get("variables", ()) + (self.var,)  #564 (line num in coconut source)
        return inner_kwargs  #565 (line num in coconut source)


    def drop_quantifier(self, nonempty_universe=True, in_forall=False, **kwargs):  #567 (line num in coconut source)
        kwargs["nonempty_universe"], kwargs["in_forall"] = nonempty_universe, in_forall  #568 (line num in coconut source)
        if not nonempty_universe and not in_forall:  #569 (line num in coconut source)
            elem = self.elem  #570 (line num in coconut source)
            while isinstance(elem, Exists):  #571 (line num in coconut source)
                elem = elem.elem  #572 (line num in coconut source)
            if top == elem:  #573 (line num in coconut source)
                return elem  #574 (line num in coconut source)
        elif self.var.is_free_in(self.elem):  #575 (line num in coconut source)
            return self.elem  #576 (line num in coconut source)
        return self  #577 (line num in coconut source)


_coconut_call_set_names(ForAll)  #579 (line num in coconut source)
FA = ForAll  #579 (line num in coconut source)


class Exists(Quantifier):  #582 (line num in coconut source)
    """Existential quantifier."""  #583 (line num in coconut source)
    __slots__ = ()  #584 (line num in coconut source)
    opstr = exists_sym  #585 (line num in coconut source)

    def __init__(self, var, elem):  #587 (line num in coconut source)
        assert wff(elem), elem  #588 (line num in coconut source)
        if isinstance(var, str):  #589 (line num in coconut source)
            var = Const(var)  #590 (line num in coconut source)
        assert isvar(var), var  #591 (line num in coconut source)
        self.var = var.constant()  #592 (line num in coconut source)
        self.elem = elem.substitute({var: self.var.constant()})  #593 (line num in coconut source)


    @_coconut_tco  #595 (line num in coconut source)
    def resolve(self, **kwargs):  #595 (line num in coconut source)
        skolem_args = kwargs.get("variables")  #596 (line num in coconut source)
        if skolem_args is None:  #597 (line num in coconut source)
            skolem_var = self.var.skolem()  #598 (line num in coconut source)
            while not skolem_var.is_free_in(self.elem):  #599 (line num in coconut source)
                skolem_var = skolem_var.prime()  #600 (line num in coconut source)
        else:  #601 (line num in coconut source)
            skolem_var = Func(self.var.skolem(), *skolem_args)  #602 (line num in coconut source)
        skolem_elem = self.elem.substitute({self.var: skolem_var})  #603 (line num in coconut source)
        return _coconut_tail_call(Exists(self.var, skolem_elem.resolve(**self.inner_kwargs(kwargs))).simplify, dnf=False, **kwargs)  #604 (line num in coconut source)


    def inner_kwargs(self, kwargs):  #606 (line num in coconut source)
        __class__ = Exists  #607 (line num in coconut source)

        inner_kwargs = super().inner_kwargs(kwargs)  #607 (line num in coconut source)
        inner_kwargs["nonempty_universe"] = True  #608 (line num in coconut source)
        return inner_kwargs  #609 (line num in coconut source)


    def drop_quantifier(self, nonempty_universe=True, in_exists=False, **kwargs):  #611 (line num in coconut source)
        kwargs["nonempty_universe"], kwargs["in_exists"] = nonempty_universe, in_exists  #612 (line num in coconut source)
        if not nonempty_universe and not in_exists:  #613 (line num in coconut source)
            elem = self.elem  #614 (line num in coconut source)
            while isinstance(elem, ForAll):  #615 (line num in coconut source)
                elem = elem.elem  #616 (line num in coconut source)
            if bot == elem:  #617 (line num in coconut source)
                return elem  #618 (line num in coconut source)
        elif self.var.is_free_in(self.elem):  #619 (line num in coconut source)
            return self.elem  #620 (line num in coconut source)
        return self  #621 (line num in coconut source)


    def admits_empty_universe(self):  #623 (line num in coconut source)
        return False  #623 (line num in coconut source)


_coconut_call_set_names(Exists)  #625 (line num in coconut source)
TE = Exists  #625 (line num in coconut source)


class BinaryOp(Expr):  #628 (line num in coconut source)
    """Base class for binary operators."""  #629 (line num in coconut source)
    __slots__ = ("elems",)  #630 (line num in coconut source)
    identity = None  #631 (line num in coconut source)

    def __new__(cls, *elems):  #633 (line num in coconut source)
        if not elems:  #634 (line num in coconut source)
            if cls.identity is None:  #635 (line num in coconut source)
                raise TypeError(cls.__name__ + " requires at least one argument")  #636 (line num in coconut source)
            else:  #637 (line num in coconut source)
                return cls.identity  #638 (line num in coconut source)
        elif len(elems) == 1:  #639 (line num in coconut source)
            assert wff(elems[0]), elems[0]  #640 (line num in coconut source)
            return elems[0]  # sometimes returns an instance of cls  #641 (line num in coconut source)
        else:  #642 (line num in coconut source)
            __class__ = BinaryOp  #643 (line num in coconut source)

            return super().__new__(cls)  #643 (line num in coconut source)


    def __init__(self, *elems):  #645 (line num in coconut source)
        if len(elems) > 1:  # __new__ should handle all other cases  #646 (line num in coconut source)
            assert len(elems) >= 2, elems  #647 (line num in coconut source)
            for x in elems:  #648 (line num in coconut source)
                assert wff(x), x  #649 (line num in coconut source)
            self.elems = elems  #650 (line num in coconut source)


    def __repr__(self):  #652 (line num in coconut source)
        return self.__class__.__name__ + "(" + ", ".join((repr(x) for x in self.elems)) + ")"  #653 (line num in coconut source)


    @_coconut_tco  #655 (line num in coconut source)
    def __str__(self):  #655 (line num in coconut source)
        return _coconut_tail_call((" " + self.opstr + " ").join, (quote(x) for x in self.elems))  #656 (line num in coconut source)


    def __len__(self):  #658 (line num in coconut source)
        return sum(map(len, self.elems)) + 1  #659 (line num in coconut source)


    @_coconut_tco  #661 (line num in coconut source)
    def substitute(self, subs, **kwargs):  #661 (line num in coconut source)
        return _coconut_tail_call((self.__class__), *(map)(_coconut.operator.methodcaller("substitute", subs, **kwargs), self.elems))  #662 (line num in coconut source)


    @_coconut_tco  #664 (line num in coconut source)
    def resolve(self, **kwargs):  #664 (line num in coconut source)
        elems = (map)(_coconut.operator.methodcaller("resolve", **kwargs), self.elems)  #665 (line num in coconut source)
        return _coconut_tail_call(self.__class__(*elems).simplify, dnf=False, **kwargs)  #666 (line num in coconut source)



_coconut_call_set_names(BinaryOp)  #669 (line num in coconut source)
class Imp(BinaryOp):  #669 (line num in coconut source)
    """Logical implication."""  #670 (line num in coconut source)
    __slots__ = ()  #671 (line num in coconut source)
    opstr = imp_sym  #672 (line num in coconut source)

    @_coconut_tco  #674 (line num in coconut source)
    def __rshift__(self, other):  #674 (line num in coconut source)
        if isinstance(other, Imp):  #675 (line num in coconut source)
            return _coconut_tail_call(Imp, self, *other.elems)  #676 (line num in coconut source)
        else:  #677 (line num in coconut source)
            return _coconut_tail_call(Imp, self, other)  #678 (line num in coconut source)


    @_coconut_tco  #680 (line num in coconut source)
    def __lshift__(self, other):  #680 (line num in coconut source)
        return _coconut_tail_call((Imp), *(other,) + self.elems)  #681 (line num in coconut source)


    @property  #683 (line num in coconut source)
    def conds(self):  #684 (line num in coconut source)
        return self.elems[:-1]  #684 (line num in coconut source)


    @property  #686 (line num in coconut source)
    def concl(self):  #687 (line num in coconut source)
        return self.elems[-1]  #687 (line num in coconut source)


    def __eq__(self, other):  #689 (line num in coconut source)
        return isinstance(other, self.__class__) and self.concl == other.concl and (unorderd_eq)(self.conds, other.conds)  #690 (line num in coconut source)


    @_coconut_tco  #692 (line num in coconut source)
    def to_or(self):  #692 (line num in coconut source)
        ors = tuple(map(Not, self.conds)) + (self.concl,)  #693 (line num in coconut source)
        return _coconut_tail_call(Or, *ors)  #694 (line num in coconut source)


    @_coconut_tco  #696 (line num in coconut source)
    def simplify(self, **kwargs):  #696 (line num in coconut source)
        return _coconut_tail_call(self.to_or().simplify, **kwargs)  #697 (line num in coconut source)


    @_coconut_tco  #699 (line num in coconut source)
    def admits_empty_universe(self):  #699 (line num in coconut source)
        return _coconut_tail_call(self.to_or().admits_empty_universe)  #700 (line num in coconut source)


_coconut_call_set_names(Imp)  #702 (line num in coconut source)
Implies = Imp  #702 (line num in coconut source)


class BoolOp(BinaryOp):  #705 (line num in coconut source)
    """Base class for Or and And."""  #706 (line num in coconut source)
    __slots__ = ()  #707 (line num in coconut source)

    def __eq__(self, other):  #709 (line num in coconut source)
        return isinstance(other, self.__class__) and (unorderd_eq)(self.elems, other.elems)  #710 (line num in coconut source)


    def simplify(self, **kwargs):  #712 (line num in coconut source)
        elems = (map)(_coconut.operator.methodcaller("simplify", **kwargs), self.merge().elems)  #713 (line num in coconut source)
        out = self.__class__(*elems).clean()  #714 (line num in coconut source)
        if isinstance(out, self.__class__):  #715 (line num in coconut source)
            out = out.distribute(**kwargs)  #716 (line num in coconut source)
        if isinstance(out, self.__class__):  #717 (line num in coconut source)
            out = out.merge().dedupe()  #718 (line num in coconut source)
        if isinstance(out, self.__class__):  #719 (line num in coconut source)
            out = out.inner_simplify(**kwargs)  #720 (line num in coconut source)
        if isinstance(out, self.__class__):  #721 (line num in coconut source)
            out = out.prenex(**kwargs)  #722 (line num in coconut source)
        log_simplification(self, out, **kwargs)  #723 (line num in coconut source)
        return out  #724 (line num in coconut source)


    @_coconut_tco  #726 (line num in coconut source)
    def merge(self):  #726 (line num in coconut source)
        """Merges nested copies of a boolean operator."""  #727 (line num in coconut source)
        elems = []  #728 (line num in coconut source)
        for x in self.elems:  #729 (line num in coconut source)
            if isinstance(x, self.__class__):  #730 (line num in coconut source)
                elems.extend(x.merge().elems)  #731 (line num in coconut source)
            else:  #732 (line num in coconut source)
                elems.append(x)  #733 (line num in coconut source)
        return _coconut_tail_call(self.__class__, *elems)  #734 (line num in coconut source)


    @_coconut_tco  #736 (line num in coconut source)
    def dedupe(self):  #736 (line num in coconut source)
        """Removes duplicate elements from a boolean operator."""  #737 (line num in coconut source)
        elems = []  #738 (line num in coconut source)
        for x in self.elems:  #739 (line num in coconut source)
            if x not in elems:  #740 (line num in coconut source)
                elems.append(x)  #741 (line num in coconut source)
        return _coconut_tail_call(self.__class__, *elems)  #742 (line num in coconut source)


    @_coconut_tco  #744 (line num in coconut source)
    def clean(self):  #744 (line num in coconut source)
        """Removes copies of the identity."""  #745 (line num in coconut source)
        return _coconut_tail_call((self.__class__), *(filter)(_coconut.functools.partial((_coconut.operator.ne), self.identity), self.elems))  #746 (line num in coconut source)


    def prenex(self, **kwargs):  #748 (line num in coconut source)
        """Pulls quantifiers out."""  #749 (line num in coconut source)
        for i, x in enumerate(self.elems):  #750 (line num in coconut source)
            if isinstance(x, Quantifier) and self.can_prenex(x, **kwargs):  #751 (line num in coconut source)
                elems = self.elems[:i] + self.elems[i + 1:]  #752 (line num in coconut source)
                free_x = x.make_free_in(self.__class__(*elems))  #753 (line num in coconut source)
                elems += (free_x.elem,)  #754 (line num in coconut source)
                return free_x.change_elem(self.__class__(*elems)).simplify(**kwargs)  #755 (line num in coconut source)
        return self  #756 (line num in coconut source)



_coconut_call_set_names(BoolOp)  #759 (line num in coconut source)
class Or(BoolOp):  #759 (line num in coconut source)
    """Logical disjunction."""  #760 (line num in coconut source)
    __slots__ = ()  #761 (line num in coconut source)
    opstr = or_sym  #762 (line num in coconut source)
    identity = bot  #763 (line num in coconut source)

    @_coconut_tco  #765 (line num in coconut source)
    def __or__(self, other):  #765 (line num in coconut source)
        return _coconut_tail_call((Or), *self.elems + (other,))  #766 (line num in coconut source)


    @property  #768 (line num in coconut source)
    def ors(self):  #769 (line num in coconut source)
        return self.elems  #769 (line num in coconut source)


    def distribute(self, dnf=False, **kwargs):  #771 (line num in coconut source)
        """If this Or contains an And, distribute into it."""  #772 (line num in coconut source)
        kwargs["dnf"] = dnf  #773 (line num in coconut source)
        if not dnf:  #774 (line num in coconut source)
            for i, x in enumerate(self.ors):  #775 (line num in coconut source)
                if isinstance(x, And):  #776 (line num in coconut source)
                    ands = ((Or)(*(y,) + self.ors[:i] + self.ors[i + 1:]) for y in x.ands)  #777 (line num in coconut source)
                    return And(*ands).simplify(**kwargs)  #778 (line num in coconut source)
        return self  #779 (line num in coconut source)


    def inner_simplify(self, nonempty_universe=True, **kwargs):  #781 (line num in coconut source)
        """Determines if the Or is a blatant tautology."""  #782 (line num in coconut source)
        kwargs["nonempty_universe"] = nonempty_universe  #783 (line num in coconut source)
        for i, x in enumerate(self.ors):  #784 (line num in coconut source)
            if top == x:  #785 (line num in coconut source)
                return x  #786 (line num in coconut source)
            for y in self.ors[i + 1:]:  #787 (line num in coconut source)
                if x.contradicts(y, **kwargs):  #788 (line num in coconut source)
                    if not nonempty_universe and not self.admits_empty_universe():  #789 (line num in coconut source)
                        return Exists.blank(top)  #790 (line num in coconut source)
                    else:  #791 (line num in coconut source)
                        return top  #792 (line num in coconut source)
        return self  #793 (line num in coconut source)


    def can_prenex(self, other, nonempty_universe=True, in_forall=False, prenex_foralls=False, prenex_exists=True, **_):  #795 (line num in coconut source)
        if not prenex_foralls and isinstance(other, ForAll):  #796 (line num in coconut source)
            return False  #797 (line num in coconut source)
        if not prenex_exists and isinstance(other, Exists):  #798 (line num in coconut source)
            return False  #799 (line num in coconut source)
        return (nonempty_universe or in_forall or not isinstance(other, Exists) or not self.admits_empty_universe())  #800 (line num in coconut source)


    @_coconut_tco  #805 (line num in coconut source)
    def resolve_against(self, other, **kwargs):  #805 (line num in coconut source)
        if isinstance(other, Eq):  #806 (line num in coconut source)
            return _coconut_tail_call(other.resolve_against, self)  #807 (line num in coconut source)
        elif isinstance(other, Or):  #808 (line num in coconut source)
            not_other_ors = (map)(_coconut.operator.methodcaller("simplify", **kwargs), (map)(Not, other.ors))  #809 (line num in coconut source)
            for i, x in enumerate(self.ors):  #810 (line num in coconut source)
                if isinstance(x, Eq):  #811 (line num in coconut source)
                    resolved_other = (x.paramodulant)(other)  #812 (line num in coconut source)
                    return (Or)(*self.ors[:i] + self.ors[i + 1:] + resolved_other.ors)  #813 (line num in coconut source)
                for j, y in enumerate(not_other_ors):  #814 (line num in coconut source)
                    if isinstance(other.ors[j], Eq):  #815 (line num in coconut source)
                        y = other.ors[j]  #816 (line num in coconut source)
                        resolved_self = (y.paramodulant)(self)  #817 (line num in coconut source)
                        return (Or)(*other.ors[:j] + other.ors[j + 1:] + resolved_self.ors)  #818 (line num in coconut source)
                    subs = x.find_unification(y, **kwargs)  #819 (line num in coconut source)
                    if subs is not None:  #820 (line num in coconut source)
                        return (Or)(*(map)(_coconut.operator.methodcaller("substitute", subs, **kwargs), self.ors[:i] + self.ors[i + 1:] + other.ors[:j] + other.ors[j + 1:]))  #821 (line num in coconut source)
        else:  #822 (line num in coconut source)
            not_other = Not(other).simplify(**kwargs)  #823 (line num in coconut source)
            for i, x in enumerate(self.ors):  #824 (line num in coconut source)
                if isinstance(x, Eq):  #825 (line num in coconut source)
                    return (x.paramodulant)((Or)(*self.ors[:i] + self.ors[i + 1:]))  #826 (line num in coconut source)
                subs = x.find_unification(not_other, **kwargs)  #827 (line num in coconut source)
                if subs is not None:  #828 (line num in coconut source)
                    return (Or)(*(map)(_coconut.operator.methodcaller("substitute", subs, **kwargs), self.ors[:i] + self.ors[i + 1:]))  #829 (line num in coconut source)
        return None  #830 (line num in coconut source)


    @_coconut_tco  #832 (line num in coconut source)
    def admits_empty_universe(self):  #832 (line num in coconut source)
        return _coconut_tail_call(any, (x.admits_empty_universe() for x in self.elems))  #833 (line num in coconut source)



_coconut_call_set_names(Or)  #836 (line num in coconut source)
class And(BoolOp):  #836 (line num in coconut source)
    """Logical conjunction."""  #837 (line num in coconut source)
    __slots__ = ()  #838 (line num in coconut source)
    opstr = and_sym  #839 (line num in coconut source)
    identity = top  #840 (line num in coconut source)

    @_coconut_tco  #842 (line num in coconut source)
    def __and__(self, other):  #842 (line num in coconut source)
        return _coconut_tail_call((And), *self.elems + (other,))  #843 (line num in coconut source)


    @property  #845 (line num in coconut source)
    def ands(self):  #846 (line num in coconut source)
        return self.elems  #846 (line num in coconut source)


    def distribute(self, dnf=False, **kwargs):  #848 (line num in coconut source)
        """If this And contains an Or, distribute into it."""  #849 (line num in coconut source)
        kwargs["dnf"] = dnf  #850 (line num in coconut source)
        if dnf:  #851 (line num in coconut source)
            for i, x in enumerate(self.ands):  #852 (line num in coconut source)
                if isinstance(x, Or):  #853 (line num in coconut source)
                    ors = ((And)(*(y,) + self.ands[:i] + self.ands[i + 1:]) for y in x.ors)  #854 (line num in coconut source)
                    return Or(*ors).simplify(**kwargs)  #855 (line num in coconut source)
        return self  #856 (line num in coconut source)


    def inner_simplify(self, nonempty_universe=True, **kwargs):  #858 (line num in coconut source)
        """Determines if the And is a blatant contradiction."""  #859 (line num in coconut source)
        kwargs["nonempty_universe"] = nonempty_universe  #860 (line num in coconut source)
        for i, x in enumerate(self.ands):  #861 (line num in coconut source)
            if bot == x:  #862 (line num in coconut source)
                return x  #863 (line num in coconut source)
            for y in self.ands[i + 1:]:  #864 (line num in coconut source)
                if x.contradicts(y, **kwargs):  #865 (line num in coconut source)
                    if not nonempty_universe and self.admits_empty_universe():  #866 (line num in coconut source)
                        return ForAll.blank(bot)  #867 (line num in coconut source)
                    else:  #868 (line num in coconut source)
                        return bot  #869 (line num in coconut source)
        return self  #870 (line num in coconut source)


    def can_prenex(self, other, nonempty_universe=True, in_exists=False, prenex_foralls=False, prenex_exists=True, **_):  #872 (line num in coconut source)
        if not prenex_foralls and isinstance(other, ForAll):  #873 (line num in coconut source)
            return False  #874 (line num in coconut source)
        if not prenex_exists and isinstance(other, Exists):  #875 (line num in coconut source)
            return False  #876 (line num in coconut source)
        return (nonempty_universe or in_exists or not isinstance(other, ForAll) or all((isinstance(x, ForAll) for x in self.elems)))  #877 (line num in coconut source)


    @_coconut_tco  #882 (line num in coconut source)
    def resolve(self, nonempty_universe=True, just_skolemize=False, debug=False, **kwargs):  #882 (line num in coconut source)
        """Performs all possible resolutions within the And."""  #883 (line num in coconut source)
        kwargs["nonempty_universe"], kwargs["just_skolemize"], kwargs["debug"] = nonempty_universe, just_skolemize, debug  #884 (line num in coconut source)

# skolemize
        __class__ = And  #887 (line num in coconut source)

        resolved = super().resolve(**kwargs)  #887 (line num in coconut source)
        log_simplification(self, resolved, **kwargs)  #888 (line num in coconut source)
        if not isinstance(resolved, And) or just_skolemize:  #889 (line num in coconut source)
            return resolved  #890 (line num in coconut source)
        self = resolved  #891 (line num in coconut source)

# if we don't admit a nonempty universe, enforce that
        quantifiers = []  #894 (line num in coconut source)
        if not nonempty_universe and not self.admits_empty_universe():  #895 (line num in coconut source)
            blank = Exists.blank(top)  #896 (line num in coconut source)
            (quantifiers.append)(blank.change_elem)  #897 (line num in coconut source)
            kwargs = (blank.inner_kwargs)(kwargs)  #898 (line num in coconut source)

# push foralls into clauses
        clauses = []  #901 (line num in coconut source)
        final_subs = {}  #902 (line num in coconut source)
        for i, clause in enumerate(self.ands):  #903 (line num in coconut source)
            clause_subs = dict(((v), (v.subscript(i))) for v in kwargs.get("variables", ()))  #904 (line num in coconut source)
            (clauses.append)(clause.substitute(clause_subs, **kwargs))  #905 (line num in coconut source)
            final_subs.update(dict(((v.subscript(i)), (v)) for v in kwargs.get("variables", ())))  #906 (line num in coconut source)
        kwargs["variables"] = (tuple)(final_subs.keys())  #907 (line num in coconut source)
        log_simplification(self, And(*clauses), **kwargs)  #908 (line num in coconut source)

# main resolution loop
        prev_clause_len = 1  #911 (line num in coconut source)
        while prev_clause_len < len(clauses):  #912 (line num in coconut source)
            prev_clause_len = len(clauses)  #913 (line num in coconut source)
# reversed ensures conclusions get tested first
            for i in (reversed)(range(1, len(clauses))):  #915 (line num in coconut source)
                x = clauses[i]  #916 (line num in coconut source)
                for y in clauses[:i + 1]:  # allow resolution of a clause against itself  #917 (line num in coconut source)
                    resolution = x.resolve_against(y)  #918 (line num in coconut source)
                    if resolution is not None:  #919 (line num in coconut source)
                        resolution = resolution.simplify(dnf=False, **kwargs)  #920 (line num in coconut source)
                        log_resolution(x, y, resolution, **kwargs)  #921 (line num in coconut source)
                        new_quantifiers = []  #922 (line num in coconut source)
                        inner_kwargs = kwargs  #923 (line num in coconut source)
                        while isinstance(resolution, Quantifier) and self.can_prenex(resolution, **kwargs):  #924 (line num in coconut source)
                            (new_quantifiers.append)(resolution.change_elem)  #925 (line num in coconut source)
                            inner_kwargs = (resolution.inner_kwargs)(inner_kwargs)  #926 (line num in coconut source)
                            resolution = resolution.elem  #927 (line num in coconut source)
                        if isinstance(resolution, And):  #928 (line num in coconut source)
                            new_clauses = resolution.ands  #929 (line num in coconut source)
                        else:  #930 (line num in coconut source)
                            new_clauses = (resolution,)  #931 (line num in coconut source)
                        novel = False  #932 (line num in coconut source)
                        for new_clause in new_clauses:  #933 (line num in coconut source)
                            if new_clause == bot:  #934 (line num in coconut source)
                                clauses = [bot,]  #935 (line num in coconut source)
                                novel = True  #936 (line num in coconut source)
                                break  #937 (line num in coconut source)
                            elif new_clause != top and new_clause not in clauses:  #938 (line num in coconut source)
                                clauses.append(new_clause)  #939 (line num in coconut source)
                                novel = True  #940 (line num in coconut source)
                        if novel:  #941 (line num in coconut source)
                            quantifiers.extend(new_quantifiers)  #942 (line num in coconut source)
                            kwargs = inner_kwargs  #943 (line num in coconut source)
                            if clauses == [bot,]:  #944 (line num in coconut source)
                                break  #945 (line num in coconut source)
                if clauses == [bot,]:  #946 (line num in coconut source)
                    break  #947 (line num in coconut source)

# combine resolved clauses
        resolved = ((reduce)(_coconut_pipe, [And(*clauses),] + quantifiers)).substitute(final_subs, **kwargs)  #950 (line num in coconut source)
        log_simplification(self, resolved, **kwargs)  #951 (line num in coconut source)
        self = resolved  #952 (line num in coconut source)

        return _coconut_tail_call(self.simplify, dnf=False, **kwargs)  #954 (line num in coconut source)


    @_coconut_tco  #956 (line num in coconut source)
    def admits_empty_universe(self):  #956 (line num in coconut source)
        return _coconut_tail_call(all, (x.admits_empty_universe() for x in self.elems))  #957 (line num in coconut source)



_coconut_call_set_names(And)  #960 (line num in coconut source)
class Eq(Expr):  #960 (line num in coconut source)
    """Equality operator."""  #961 (line num in coconut source)
    __slots__ = ("a", "b")  #962 (line num in coconut source)
    ...
    def find_unification(self, other, **kwargs):  #988 (line num in coconut source)
        if isinstance(other, Eq):  #989 (line num in coconut source)
            a_a = self.a.find_unification(other.a, **kwargs)  #990 (line num in coconut source)
            b_b = self.b.find_unification(other.b, **kwargs)  #991 (line num in coconut source)
            if a_a is not None and b_b is not None:  #992 (line num in coconut source)
                subs = merge_dicts(a_a, b_b)  #993 (line num in coconut source)
                if subs is not None:  #994 (line num in coconut source)
                    return subs  #995 (line num in coconut source)
            a_b = self.a.find_unification(other.a, **kwargs)  #996 (line num in coconut source)
            b_a = self.b.find_unification(other.b, **kwargs)  #997 (line num in coconut source)
            if a_b is not None and b_a is not None:  #998 (line num in coconut source)
                subs = merge_dicts(a_b, b_a)  #999 (line num in coconut source)
                if subs is not None:  #1000 (line num in coconut source)
                    return subs  #1001 (line num in coconut source)
        else:  #1002 (line num in coconut source)
            __class__ = Eq  #1003 (line num in coconut source)

            return super().find_unification(other, **kwargs)  #1003 (line num in coconut source)


    @_coconut_tco  #1005 (line num in coconut source)
    def substitute(self, subs, **kwargs):  #1005 (line num in coconut source)
        return _coconut_tail_call(Eq, self.a.substitute(subs, **kwargs), self.b.substitute(subs, **kwargs))  #1006 (line num in coconut source)


    @_coconut_tco  #1008 (line num in coconut source)
    def paramodulant(self, other):  #1008 (line num in coconut source)
        """Create a paramodulant of other."""  #1009 (line num in coconut source)
        return _coconut_tail_call((sub_once), other, {self.a: self.b, self.b: self.a})  #1010 (line num in coconut source)


    @_coconut_tco  #1012 (line num in coconut source)
    def resolve_against(self, other, **kwargs):  #1012 (line num in coconut source)
        if isinstance(other, Not) and self.find_unification(other.neg, **kwargs) is not None:  #1013 (line num in coconut source)
            return bot  #1014 (line num in coconut source)
        else:  #1015 (line num in coconut source)
            return _coconut_tail_call(self.paramodulant, other)  #1016 (line num in coconut source)


    def admits_empty_universe(self):  #1018 (line num in coconut source)
        return self.a.admits_empty_universe() and self.b.admits_empty_universe()  #1019 (line num in coconut source)


_coconut_call_set_names(Eq)  #1021 (line num in coconut source)
Equals = Eq  #1021 (line num in coconut source)
